...
export default function EditCharacterPage() {
    ...
    const fetchCharacter = async () => {
        try {
            ...
            console.log('📦 Character data from backend:', data);
            setFormData({
                name: data.name || '',
                description: data.description || '',
                systemPrompt: data.systemPrompt || data.personality || '', // Handle both fields
                category: data.category || 'Original',
                voiceId: data.voiceId || data.voice || '', // Handle both fields
            ...
            if (data.avatar || data.image) {
                setPreviewUrl(data.avatar || data.image);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await characterAPI.updateCharacter(characterId, data);
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center space-y-4">
                    <Loader2 className="w-12 h-12 animate-spin text-gray-900 dark:text-white mx-auto" />
                    <p className="text-sm text-gray-500 dark:text-gray-400">Loading character...</p>
                </div>
            </div>
        );
    }
                            ...
                            <div className="flex-shrink-0">
                                {previewUrl ? (
                                    <div className="relative w-32 h-32 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-800">
                                        <Image src={previewUrl} sizes="128px" alt="Preview" fill className="object-cover" />
                                        <button
                                            type="button"
                                            onClick={() => {
                                                ...
                                                {filteredVoices.length > 0 ? (
                                                    filteredVoices.map(voice => (
                                                        <button
                                                            ...
                                                            className={`w-full px-3 py-2 flex items-center gap-3 hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors text-left ${formData.voiceId === voice.id ? 'bg-gray-50 dark:bg-gray-900' : ''
                                                                }`}
                                                        >
                                                            <div className={`w-8 h-8 rounded-md flex items-center justify-center ${voice.gender === 'Female'
